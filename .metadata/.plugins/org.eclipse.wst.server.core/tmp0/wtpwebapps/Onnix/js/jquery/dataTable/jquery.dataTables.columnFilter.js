...
(function ($) {
    ...
    function fnCreateInput(regex, smart, bIsNumber) {
        ...
        var input = $('<input type="text" class="search_init input-mini search-query ' + sCSSClass + '" value="' + label + '"/>');
        th.html(input);
        if (bIsNumber)
        ...
        if (bIsNumber && !oTable.fnSettings().oFeatures.bServerSide) {
            input.keyup(function () {
                ...
                oTable.fnFilter('^' + this.value, index+1, true, false);
            });
        } else {
            input.keyup(function () {
                /* Filter on the column (the index) of this element */
                oTable.fnFilter(this.value, index+1, regex, smart);
            });
        }
    ...
    function fnCreateSelect(aData) {
        var index = i;
        var r = '<select class="search_init select_filter input-mini search-query"><option value="" class="search_init">' + label + '</option>', j, iLen = aData.length;
        ...
        select.change(function () {
            ...
            oTable.fnFilter($(this).val(), index+1);
        });
    }
